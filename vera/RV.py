...
class RV:
    ...
    def change_instrument_names(self, old, new):
        ...
        if not isinstance(new, str):
            new = str(new)
        if new[0].isdigit():
            error(f"New nane \"{name}\" starts with a digit. That doesn't work...")
            return
    ...
    def _update_stellar_parameters(self):
        download_star_parameters_file()
        self.prot, self.teff
        del self.prot, self.teff
    ...
    def _known_planets_signals(self, adjust_tp=True):
        self.known_planets
        d = self.known_planets_data

        du = UnitSystem(u.au, u.day, u.Msun, u.degree, u.m / u.s)
        P = d['orbital_period'] * u.day

        adjust_omega = False
        if np.isnan(d['tperi']).any():
            omega = np.zeros_like(d['omega']) * u.deg
            adjust_omega = True
            if self.verbose:
                print(yelb | 'warning:', end=' ')
                print('argument of periastron is NaN in exoplanet.eu,',
                      end=' ')
                print('going to fit it.')
        else:
            omega = d['omega'] * u.deg

        A = d['semi_major_axis'] * u.au
        K = d['k'] * du['speed']

        if np.isnan(d['tperi']).any():
            Tp = self.time[0] * np.ones_like(d['tperi']) * u.day
            adjust_tp = True
            if self.verbose:
                print(yelb | 'warning:', end=' ')
                print('time of periastron is NaN in exoplanet.eu,', end=' ')
                print('going to fit it.')
        else:
            if (d['tperi'] > 24e5).any():
                d['tperi'] -= 24e5
        ...
        ninst = self.instruments.size
        orbs = []
        for p, w, a, tp, k in zip(P, omega, A, Tp, K):
            orbs.append(KeplerOrbit(P=p, omega=w, t0=tp, i=90 * u.deg))
            orbs[-1].K = k

        def f0(t, *pars):  # adjust offsets only
            v = [o.radial_velocity(Time(t, format='mjd')).value for o in orbs]
            v = np.sum(np.array(v), axis=0)
            return v + np.array(pars)[(self.obs - 1).astype(int)]

        def f1(t, *pars):  # adjust Tp and offsets
            v = []
            for i, orb in enumerate(orbs):
                orb.t0 = Time(pars[i], format='mjd')
                v.append(orb.radial_velocity(Time(t, format='mjd')).value)
            v = np.sum(np.array(v), axis=0)
            return v + np.array(pars[-ninst:])[(self.obs - 1).astype(int)]

        def f2(t, *pars):  # adjust Tp, omega, and offset
            print(pars)
            NP = len(orbs)
            end = self.instruments.size
            for i, (t0, om) in enumerate(zip(pars[:-1], pars[NP:-end])):
                orbs[i].t0 = Time(t0, format='mjd')
                orbs[i].omega = om * u.deg
        ...
        if adjust_tp and adjust_omega:
            p0 = [o.t0.value for o in orbs] + [o.omega.value for o in orbs]
            p0 += [0.] * self.instruments.size
            # p0 += [s.vrad.mean() for s in self.each]

            bounds = []
            for _ in orbs:
                bounds += [(self.time.min(), self.time.max())]
            for _ in orbs:
                bounds += [(0, 360)]

            bounds += [(-50, 50) for s in self.each]
            bounds = tuple(b[0] for b in bounds), tuple(b[1] for b in bounds)

            print(bounds)
            print(p0)
            popt, pcov = optimize.curve_fit(f2, self.time, self.vrad,
                                            sigma=self.svrad, method='trf',
                                            p0=p0, bounds=bounds)

            # return
            NP = len(orbs)
            end = self.instruments.size
            for i, (t0, om) in enumerate(zip(popt[:-1], popt[NP:-end])):
                orbs[i].t0 = Time(tp, format='mjd')
                orbs[i].omega = om * u.deg

        elif adjust_tp:
            p0 = [o.t0.value for o in orbs] + [0.] * self.instruments.size
            print(p0)
            ...
            for orb, tp in zip(orbs, popt):
                orb.t0 = Time(tp, format='mjd')
        else:
            p0 = np.zeros_like(self.instruments, dtype=float)
            popt, pcov = optimize.curve_fit(
                f0, self.time, self.vrad, sigma=self.svrad, method='lm', p0=p0)
    ...
    def plot_known_planets(self, include_sigma_clip=False, adjust_tp=True):
        du = UnitSystem(u.au, u.day, u.Msun, u.degree, u.m / u.s)

        orbs, popt = self._known_planets_signals(adjust_tp)

        ax, _ = self.plot(include_sigma_clip=include_sigma_clip)
        tt = Time(self.time.min() - 5000, format='mjd')
        tt += np.linspace(0, self.time.ptp() + 10000, 5000)

        kep = np.zeros_like(tt) * du['speed']
        for orb in orbs:
            v = orb.radial_velocity(tt)
            v <<= du['speed']
            kep += v
            ax.plot(tt.value, v, color='k', lw=1, alpha=0.3)

        ax.set_prop_cycle(None)
        for off in popt[-self.instruments.size:]:  #[::-1]:
            ax.plot(tt.value, kep.value + off, lw=1)

        return orbs, popt

    def remove_known_planets(self, adjust_tp=False, plot=True):
        from keplerian._classes import Star, Planet, PlanetarySystem
        sys.path.append('known_planets')
        from database import known, translate

        if self.star not in known:
            return 0

        ts = known[self.star]
        d, o = ts['orb_desc'], ts['orb']
        NP = len(o)
        # "flip" it
        o = [[oo[i] for oo in o] for i in range(len(o[0]))]
        arg_names = ('P', 'K', 'e', 'omega', 'T0')

        args = {
            translate[k]: v
            for k, v in zip(d, o) if translate[k] in arg_names
        }

        planets = Planet.multi(**args)

        PS = PlanetarySystem(Star(self.stellar_mass), planets)
        # return PS

        for i, inst in enumerate(self.each):
            mask = self.obs == i + 1
            if self.verbose:
                print(f'adjusting offset for {self.instruments[i]}', end='')
            m = inst.mask
            off = PS.fit_offset(inst.time[m], inst.vrad[m], inst.svrad[m],
                                set_vsys=True)

            if self.verbose:
                print(f'got {off:.2f} m/s')

            if plot:
                fig, ax = plt.subplots()
                inst.plot(ax=ax)
                tt = np.linspace(inst.time[0], inst.time[-1], 3000)
                ax.plot(tt, PS.rv(tt), 'k')

            # subtract the known planets
            # if self.verbose:
            #     print(f' -> subtracting known planets')

            self.vrad[mask] -= PS.rv(self.time[mask])
            inst.vrad -= PS.rv(inst.time)
        ...
        # if plot:
        #     orbs, popt = self.plot_known_planets()
        # else:
        #     orbs, popt = self._known_planets_signals(adjust_tp)

        # t = Time(self.time, format='mjd')
        # kep = np.zeros_like(t) * du['speed']

        # for orb in orbs:
        #     v = orb.radial_velocity(t)
        #     v <<= du['speed']
        #     kep += v

        # ni = self.instruments.size
        # i = (self.obs - 1).astype(int)
        # model = kep.value + popt[-ni:][i]
        # self.vrad -= model
    ...
    def add_keplerian(self, *, P=None, K=None, plot=True, **kwargs):
        # if inputs don't have units, convert them to the default ones
        # note: the <<= operator **converts** units
        #       the *= operator multiplies (duh!)
        du = UnitSystem(u.au, u.day, u.Msun, u.degree, u.m / u.s)

        if P is None:
            raise ValueError('You must supply the orbital period P')
        P <<= du['time']

        if K is None:
            raise ValueError('You must supply the semi-amplitude K')
        K <<= du['speed']

        if 'e' in kwargs or 'ecc' in kwargs:
            if not ('w' in kwargs or 'omega' in kwargs):
                raise ValueError(
                    'You must supply the argument of pericenter w')
            if 'ecc' in kwargs:
                kwargs['e'] = kwargs['ecc']  # settle with 'e'
            if 'w' in kwargs:
                kwargs['omega'] = kwargs['w']  # settle with 'omega'
                kwargs.pop('w')
        else:
            kwargs['e'] = 0.
            if 'w' in kwargs:
        ...
        if 'i' in kwargs or 'inc' in kwargs:
            if 'inc' in kwargs:
                kwargs['i'] = kwargs['inc']  # settle with 'i'
        else:
            kwargs['i'] = 90

        kwargs['omega'] <<= du['angle']
        kwargs['i'] <<= du['angle']
        if 'M0' in kwargs:
            kwargs['M0'] <<= du['angle']

        e, i = kwargs['e'], kwargs['i']
        kwargs['a'] = K * P * np.sqrt(1 - e**2) / (2 * np.pi * np.sin(i))
        kwargs['a'] <<= du['length']
        ...
        #     s = traceback.format_exc(chain=False, limit=1)
        #     failed_par = re.findall(r"kwargs\[\'(\w)\'\]", s)[0]
        #     # fail more gracefully
        #     print(redb | f'{type(e).__name__}:', end=' ')
        #     print(f'when converting {failed_par}: {e}')
        #     return

        orb = KeplerOrbit(P=P, **kwargs)

        if not plot:
            return

        ax, _ = self.plot()
        tt = Time(self.time.min(), format='mjd')
        tt += np.linspace(0, self.time.ptp(), 10000)

        kep = np.zeros_like(tt) * du['speed']
        for previous in self.keplerians:
            v = previous.radial_velocity(tt)
            v <<= du['speed']
            kep += v
        # ax.plot(tt.value, kep, color='k', lw=1)

        v = orb.radial_velocity(tt)
        v <<= du['speed']
        kep += v
        ax.plot(tt.value, kep, color='r', lw=1, alpha=0.5)
        self.keplerians.append(orb)
    ...
    def detection_limits(self, star_mass=1.0, WN=None, plot=True, ax=None,
                         force=False, avoid=False, current=False, lines=None,
                         random_in_P=None, time_limit=None, title=True,
                         plot_kwargs={}):
        # if 'ESPRESSO' not in self.instruments:
        #     if self.verbose:
        #         print(red | 'Detection limits only work with ESPRESSO data!')
        #     return

        ESPRESSO = self #.each.ESPRESSO

        if ESPRESSO.time.size < 2:
            print(red | 'Need at least 2 ESPRESSO points!')
            return

        if self.stellar_mass == 0:
            print(red | 'Please set .stellar_mass before running this')
            return


        file = f"{self.star.replace(' ','')}.dl.pickle"

        today = Time.now().jd - 24e5

        if random_in_P is None:
            if self.verbose:
                print(blue | 'Getting GTO runs for the future periods')
            # starts, ends = pickle.load(open('runs.pickle', 'rb'))
            starts, ends = self.future_runs

            # available runs starting tonight
            available = np.array(
                [(s, e) for s, e in zip(starts, ends) if s > today])

        else:
            Period, N = random_in_P
            if self.verbose:
                print(
                    blue
                    | f'Assuming {N} new observations in ESO period {Period}',
                    end='')
            # Pstart, Pend = ESO_periods[Period][2]
            # starts = np.arange(Pstart, Pend)[:-1]
            # ends = np.arange(Pstart, Pend)[1:]
            # available = np.array(
            #     [(s, e) for s, e in zip(starts, ends) if s > today])
            available = get_random_times(None, Period, return_available=True)
            if time_limit is not None:
                print(blue | f', up until mjd={time_limit}')
                available = available[available[:, 0] < time_limit]
            else:
                print()

        n_available = len(available)
        if self.verbose:
            print(f'  ({n_available} available nights)')

        try:
            if force:
                raise Exception

            h, Nsim, sim_sys, WN = pickle.load(open(file, 'rb'))

            if h == self.hash:
                if self.verbose:
                    print(blue | f'Found detection limits in "{file}"')
                self.simulated_systems = sim_sys
            else:
                if avoid:
                    return
            ...
            self.secular_acceleration(plot=False)
            self._run_treament_steps(plot=False)

            # previous data from other instruments, not ESPRESSO
            prevtime, prevvrad, prevsvrad = [], [], []
            for i, individual in enumerate(self.each):
                if 'ESPRESSO' in individual.instruments[0]:
                    continue
                m = individual.mask & self.mask[self.obs == i + 1]
                prevtime.append(individual.time[m])
                prevvrad.append(individual.vrad[m])
                prevsvrad.append(individual.svrad[m])

            if prevtime != []:  # no previdous data?
                prevtime = np.concatenate(prevtime)
                prevvrad = np.concatenate(prevvrad)
                prevsvrad = np.concatenate(prevsvrad)

            # already available ESPRESSO data
            esp, m = ESPRESSO, ESPRESSO.mask
            t, y, e = esp.time[m], esp.vrad[m], esp.svrad[m]

            data = e  # self.each.ESPRESSO.svrad[self.each.ESPRESSO.mask]
            # fit a distribution to the observed errorbars of ESPRESSO (either log-normal or exponential)
            # exponential seems to fit better in most cases
            # error_dist = stats.lognorm(*stats.lognorm.fit(data))
            error_dist = stats.expon(*stats.expon.fit(data))
            # plot ?
            # plt.hist(data, density=True)
            # print(data)
            # eee = np.linspace(e.min(), e.max(), 1000)
            # plt.plot(eee, error_dist.pdf(eee))
            # plt.show()
            # return

            # use the std of the ESPRESSO RVs to generate new ones (or provided WN)
            WN = WN or y.std()

            if self.verbose:
                print(yel | f'Assuming white noise std={WN:.3f} {self.units}')

            if random_in_P:
                Nsim = np.atleast_1d(random_in_P[1])
            else:
                # Nsim = (20, 40, n_available)
                # Nsim = tuple(np.unique(np.clip(Nsim, 0, n_available)))
                Nsim = range(20, n_available + 1, 20)

            simulated_systems = []

            if current:
                d = do_detection_limits(self, plot=False)
                self.d = d
                simulated_systems.append(d)

            for n in Nsim:
                try:
                    chosen = np.random.choice(n_available, n, replace=False)
                except ValueError:
                    # print(red
                    #       | f'Not enough available nights: ({n_available})')
                    raise ValueError(
                        f'Not enough available nights: ({n_available} < {n})'
                    ) from None
                chosen.sort()

                # start with ESPRESSO data
                # sim_times = list(t)
                # sim_rvs = list(y)
                # sim_errs = list(e)

                # start with previous data plus ESPRESSO data
                sim_times = list(np.concatenate([t, prevtime]))
                sim_rvs = list(np.concatenate([y, prevvrad]))
                sim_errs = list(np.concatenate([e, prevsvrad]))

                for ch in available[chosen]:
                    # simulate time
                    st = np.random.uniform(*ch)
                    sim_times.append(st)
                    # simulate RV
                    sv = np.random.normal(0, WN)
                    sim_rvs.append(sv)
                    # simulate error
                    # se = stats.lognorm(0.3, scale=WN).rvs()
                    se = error_dist.rvs()
                    sim_errs.append(se)

                # ind = np.argsort(sim_times)
                # sim_times = np.array(sim_times)[ind]
                # sim_rvs = np.array(sim_rvs)[ind]
                # sim_errs = np.array(sim_errs)[ind]

                z = np.zeros_like(sim_times)
                inst = np.append(self.instruments, 'simulated')
                obs = np.r_[self.obs, (self.obs.max() + 1) * np.ones(n)]

                s = RV.from_arrays(sim_times, sim_rvs, sim_errs, z, z,
                                   ms=False, star=self.star, sigmaclip=False,
                                   adjust_means=False)
                s.instruments = inst
                s.obs = obs
                s.stellar_mass = self.stellar_mass
                # plot ?
                # s.plot(ax=plt.gca())
                # return
                d = do_detection_limits(s, plot=False)
                s.d = d
                simulated_systems.append(s)

            self.simulated_systems = simulated_systems
            pickle.dump((self.hash, Nsim, simulated_systems, WN),
                        open(file, 'wb'), protocol=-1)
        ...
        for s in self.simulated_systems:
            try:
                ax.plot(s.d['trial_periods'], s.d['det_masses'], '-', lw=1.5)
            except AttributeError:
                ax.plot(s['trial_periods'], s['det_masses'], '-', lw=1.5)

        d = self.simulated_systems[-1].d
        if self.HZ is not None:
            region = (d['trial_periods'] > self.HZ[0]) & (d['trial_periods'] <
                                                        self.HZ[1])
            ...
            # if yfit < 10:
            ax.hlines(y=yfit, xmin=self.HZ.mean(), xmax=500, color='m')
            ax.text(550, yfit, str(round(yfit, 1)) + ' $M_\oplus$')

        if current:
            leg = ['current'] + [f'+{n} ESPRESSO points' for n in Nsim]
            ax.legend(leg, loc='upper left')
        else:
            leg = [f'+{n} ESPRESSO points' for n in Nsim]
            ax.legend(leg, loc='upper left')
        ax.grid(True, alpha=0.1)

        # d = self.simulated_systems[0].d
        trial_periods = d['trial_periods']
        detected_masses_01ms = d['mass01ms']
        detected_masses_1ms = d['mass1ms']
        # # detected_masses_3ms = d['mass3ms']
        # # detected_masses_5ms = d['mass5ms']
        # # detected_masses_7ms = d['mass7ms']
        options = {'color': 'r', 'ls': '--', 'lw': 0.6, 'alpha': 0.7}
        if lines is None:
            ax.plot(trial_periods, detected_masses_01ms, **options)
            ax.plot(trial_periods, detected_masses_1ms, **options)
        elif type(lines) in (list, tuple, np.ndarray):

            def massXms(x):
                msun = 1.98892e30  # in kg
                G = 6.67300e-11
                return ((trial_periods * 24. * 3600. *
                         (self.stellar_mass * msun)**2) /
                        (2 * np.pi * G))**(1 / 3.) * x / 5.9736e24

            for l in lines:
                ax.plot(trial_periods, massXms(l), **options)

        # label_next_to_line(trial_periods, detected_masses_1ms, '1 m/s',
        #                    ax, color='r')
        ...
        # # ax2.plot(trial_periods, detected_masses_7ms, 'r--', lw=0.6, alpha=0.7, label='7m/s')
        # # ax2.axvline(x=d['tspan'], color='k', ls='--', lw=2)
        ax.set_xscale('log')

        ax.set_xlim(trial_periods.min(), trial_periods.max())
        ax.set_ylim(0, 20)

        if self.HZ is not None:
            ax.axvspan(*self.HZ, color='g', alpha=0.2)

        ax.set(xlabel='Period [d]', ylabel=r'Mass [M$_\oplus$]')

        _ = self.future_runs  # cache it
        ax.set_title(f'assuming wn {WN:.2f} m/s', #, ' \
                    #  f'periods {self._future_runs_periods}, ' \
                    #  f'{n_available} available nights ',
                     loc='right', fontsize=10)
        if title:
            ax.set_title(s.star, loc='left', fontsize=10)

        # ni = len(self.instruments)
        # ax.set_title(f'{ni} instrument' + ('s' if ni > 1 else ''), loc='left',
        #              fontsize=10)
        ax.set(**plot_kwargs)
        return fig, ax
